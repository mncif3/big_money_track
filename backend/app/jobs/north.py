...
async def daily_north_summary_job():
    async with async_session() as sess:
        job_log = EtlJobLog(job_name="daily_north_summary", status="running")
        sess.add(job_log); await sess.commit()
        try:
            data_source_mgr.init_default()
            df, src = await data_source_mgr.call("fetch_north_summary")
            today = date.today()
            ...
            stmt = insert(NorthCapitalDaily).values(**rec).on_conflict_do_update(
...
async def intraday_north_capital_job():
    await daily_north_summary_job()
